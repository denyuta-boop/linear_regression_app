...
st.markdown("Upload a CSV file or use the demo data to perform a detailed regression analysis and make predictions.")
...
with st.sidebar:
    ...
    # --- ★★★ New Feature: Use Demo Data ★★★ ---
    use_demo_data = st.checkbox("Load Demo Data ('tips' dataset)")
...
# --- Main App Logic ---
# The rest of the app will only run if a DataFrame is loaded
if df is not None:
    ...
    with st.sidebar:
        ...
        # Try to pre-select columns that are good for demo data
        default_x = "total_bill" if "total_bill" in columns else columns[0]
        default_y = "tip" if "tip" in columns else columns[1] if len(columns) > 1 else columns[0]
        
        x_column = st.selectbox('Select the X-axis (Independent Var.)', columns, index=columns.index(default_x))
        y_column = st.selectbox('Select the Y-axis (Dependent Var.)', columns, index=columns.index(default_y))

        st.markdown('---')
        st.header("3. Plot Customization")
        st.subheader("General Theme")
        palette_choice = st.selectbox("Select a color palette", PALETTE_LIST)
        ...
        marker_options = {"Circle ('o')": "o", "Point ('.')": ".", "Pixel (',')": ",", "Triangle Up ('^')": "^", "Triangle Down ('v')": "v", "Triangle Left ('<')": "<", "Triangle Right ('>')": ">", "Square ('s')": "s", "Diamond ('D')": "D", "Thin Diamond ('d')": "d", "Plus ('+')": "+", "Cross ('x')": "x", "Star ('*')": "*"}
        ...
        linestyle_options = {"Solid (-)": "-", "Dashed (--)": "--", "Dotted (:)": ":", "Dash-dot (-.)": "-."}
        ...
        title_fontsize, label_fontsize, legend_fontsize, tick_fontsize = st.slider('Title Font Size', 8, 40, 18), st.slider('Axis Label Font Size', 6, 30, 12), st.slider('Legend Font Size', 6, 30, 10), st.slider('Tick Label Font Size', 5, 25, 10)
    ...
    try:
        if platform.system() == 'Windows':
            ...
            jp_title_font, jp_label_font, jp_legend_font = FontProperties(fname=font_path, size=title_fontsize), FontProperties(fname=font_path, size=label_fontsize), FontProperties(fname=font_path, size=legend_fontsize)
    ...
    try:
        ...
        # Create a temporary dataframe for analysis, dropping rows with missing values in selected columns
        analysis_df = pd.DataFrame({x_column: x_data, y_column: y_data}).dropna()
        x_data_clean = analysis_df[x_column]
        y_data_clean = analysis_df[y_column]

        if x_data_clean.empty or y_data_clean.empty or len(x_data_clean) < 2:
            ...
            with st.sidebar:
                ...
                x_min, x_max, y_min, y_max = x_data_clean.min(), x_data_clean.max(), y_data_clean.min(), y_data_clean.max()

                if use_manual_limits:
                    st.subheader("Manual Limits")
                    col1_limit, col2_limit = st.columns(2)
                    with col1_limit: x_limit_min, y_limit_min = st.number_input("X-min", value=x_min), st.number_input("Y-min", value=y_min)
                    with col2_limit: x_limit_max, y_limit_max = st.number_input("X-max", value=x_max), st.number_input("Y-max", value=y_max)
                    x_limits, y_limits = (x_limit_min, x_limit_max), (y_limit_min, y_limit_max)
                else:
            ...
            X = sm.add_constant(x_data_clean)
            model = sm.OLS(y_data_clean, X).fit()
            ...
            with col1_main:
                ...
                sns.scatterplot(x=x_data_clean, y=y_data_clean, ax=ax, label='Actual Data', color=scatter_fill_color, s=scatter_size, marker=marker_style, edgecolor=scatter_edge_color, linewidth=scatter_edge_width)
                
                plot_df = pd.DataFrame({'x': x_data_clean, 'y_pred': y_pred})
            ...
            with col2_main:
                ...
                r_value = x_data_clean.corr(y_data_clean)
                ...
                with st.expander("See detailed explanations of metrics"):
                    ...
                    **Correlation Coefficient (r)**
                    ...
                    - **How to interpret:** Close to **+1**: Strong **positive correlation**. As one variable increases, the other also increases. Close to **-1**: Strong **negative correlation**. As one variable increases, the other decreases. Close to **0**: Little to no linear correlation.
                    ---
                    ...
                    **P-value (P>|t|)**
                    ...
                    - **How to interpret:** A **small p-value** (typically < 0.05) suggests: "This is very unlikely to happen by chance. Therefore, the relationship we're seeing is likely real and **statistically significant**." A **large p-value** (typically > 0.05) suggests: "A relationship this strong could easily happen by random chance. We cannot conclude that a real relationship exists."
                    """)
        ...
        st.error(f"An error occurred. Please check the selected columns or data format. Error: {e}")

else:
    st.info("To get started, please load data using one of the options in the sidebar.")